...
//
//
var uniSegmentedControl = function uniSegmentedControl() {return __webpack_require__.e(/*! import() | components/uni-segmented-control/uni-segmented-control */ "components/uni-segmented-control/uni-segmented-control").then(__webpack_require__.bind(null, /*! @/components/uni-segmented-control/uni-segmented-control.vue */ "E:\\Desktop\\liandu_app\\liandu_app\\components\\uni-segmented-control\\uni-segmented-control.vue"));};var _default = { components: { uniSegmentedControl: uniSegmentedControl }, data: function data() {return { bookArray: ['上传时间', '阅读量'], journaArray: ['上架时间', '阅读量'], courseArray: ['上架时间', '阅读量'], bookIndex: 0, journaIndex: 0, courseIndex: 0, bookList: [], journalList: [], items: ['主页', '图书', '杂志', '课程'], current: 0, requiredBooks: { "page_index": 1, "page_size": 10, "table_id": "13", "sort": "createtime" }, requiredJournal: { "page_index": 1, "page_size": 10, "table_id": "13", "resource_type": 2, "sort": "createtime" } };}, onLoad: function onLoad(e) {var isVip = e.joinedState;if (isVip == 'init') {uni.showModal({ title: '提示', showCancel: false, confirmText: '关闭', content: '您已申请企业会员，请等待审核。', success: function success(res) {if (res.confirm) {console.log('用户点击关闭');}} });} else if (isVip == 'notVip') {uni.showModal({ title: '提示', confirmText: '免费申请', cancelText: '我先看看', content: '为了更好的享受企业服务，请先申请为企业会员', success: function success(res) {if (res.confirm) {uni.navigateTo({ url: './applyMember' });} else if (res.cancel) {console.log('用户点击取消');}} });} else if (isVip == 'pass') {console.log('hello vip');}}, methods: { bookSearchChange: function bookSearchChange(e) {console.log('picker发送选择改变，携带值为', e.target.value);this.bookIndex = e.target.value;if (e.target.value == '1' || e.target.value == 1) {this.requiredBooks.sort = 'hit';} else if (e.target.value == '0' || e.target.value == 0) {this.requiredBooks.sort = 'createtime';}(0, _methods.post)('/book/book/page', this.requiredBooks).then(function (res) {console.log(res);}, function (err) {//异步错误处理
      });}, journaSearchChange: function journaSearchChange(e) {console.log('picker发送选择改变，携带值为', e.target.value);this.journaIndex = e.target.value;if (e.target.value == '1' || e.target.value == 1) {this.requiredJournal.sort = 'hit';} else if (e.target.value == '0' || e.target.value == 0) {this.requiredJournal.sort = 'createtime';}(0, _methods.post)('/book/book/page', this.requiredJournal).then(function (res) {console.log(res);}, function (err) {//异步错误处理
      });}, courseSearchChange: function courseSearchChange(e) {console.log('picker发送选择改变，携带值为', e.target.value);this.courseIndex = e.target.value;}, onClickItem: function onClickItem(index) {var _this = this; //console.log(index)
      if (this.current !== index) {this.current = index;}if (index == 1) {console.log('111');uni.request({ url: 'http://192.168.0.210:9999/book/book/page', method: 'POST', data: this.requiredBooks, header: { 'content-type': 'application/json' }, success: function success(res) {console.log(res);_this.bookList = res.data.data.pageBooks;} });} else if (index == 2) {console.log('222');uni.request({ url: 'http://192.168.0.210:9999/book/book/page', method: 'POST', data: this.requiredJournal, header: { 'content-type': 'application/json' }, success: function success(res) {console.log(res.data.data.pageBooks);_this.journalList = res.data.data.pageBooks;} });}}, toEnterprise: function toEnterprise() {uni.reLaunch({ url: '../index/enterprise/enterprise' });} } };exports.default = _default;
